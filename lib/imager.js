...
Imager.prototype = {
  ...
  upload: function (files, callback, variant, directory) {
    ...
    if (!fs.existsSync(tempDir + directory)) {
      ...
      for (var i = 0, str = tempDir; i < len; i++) {
        str += pathArr[i] + '/';
    ...
    var prepare = function (file, fn) {
      ...
      self.prepareUpload(file, filename, directory, variant, fn);
    };
  ...
  prepareUpload: function (file, filename, directory, variant, fn) {
    ...
    if (variants.resize) {
      Object.keys(variants.resize).forEach(function (name) {
        var processFiles = function (cb) {
          ...
          self.resizeFile(file, preset, filename, directory, cb);
        };
  ...
  resizeFile: function (file, preset, fname, directory, cb) {
    var ct = {
      'image/jpeg': '.jpg',
      'image/png': '.png',
      'image/gif': '.gif'
    ...
    var filename = fname;
    var self = this;
    var remoteFile = directory + '/' + filename + '_' + preset.name + fileType;
